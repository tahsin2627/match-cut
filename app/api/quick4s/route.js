...
export async function POST(req) {
  try {
    ...
    const css = `
      ...
      .chip { position:absolute; bottom:8%; padding:14px 26px; border-radius:14px; font-family:Montserrat,Arial,sans-serif; font-weight:800; font-size:72px; color:#fff; background:linear-gradient(135deg,#7C3AED,#FF5CAA); box-shadow:0 12px 34px rgba(0,0,0,0.4); overflow:hidden; }
      .chip .label { position:relative; z-index:2; }
      .chip .fill { position:absolute; left:0; top:0; bottom:0; width:0%; background:rgba(255,255,255,0.22); animation:fill 3.2s linear forwards 0.4s; z-index:1; }
      @keyframes fill { from { width:0%; } to { width:100%; } }
      @media (max-aspect-ratio:10/16) { .chip { font-size:60px; } }
    `;

    const clip = { asset: { type: "html", html, css, width: dims.w, height: dims.h }, start: 0, length, position: "center", transition: { in: "fade", out: "fade" } };
    const timeline = { background: "#000000", tracks: [{ clips: [clip] }] };
